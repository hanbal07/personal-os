  ...
  return (
    <AppShell>
      <div className="space-y-6">
        ...
        {loading ? (
          <Card><CardContent className="p-6 text-sm text-zinc-500">Crunching your numbers…</CardContent></Card>
        ) : (
          <>
              ...
              <Card>
                <CardContent className="p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-zinc-500">Learning ({summary?.totalDays ?? 7}d)</p>
                      <p className="text-xl font-bold text-white">{summary?.totalLearningHours ?? 0}h</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-zinc-500">Avg Daily Learning</p>
                      <p className="text-xl font-bold text-white">{summary?.avgDailyLearning ?? 0}h</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold text-white">{summary ? Math.round(summary.avgDiscipline) : 0}%</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-zinc-500">Missed Prayer Logs</p>
                      <p className="text-xl font-bold text-white">{missedPrayers}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <Card>
                    ...
                    <CardContent>
                      {weeklyHours.length === 0 ? (
                        <p className="py-16 text-center text-sm text-zinc-600">No learning sessions logged yet.</p>
                      ) : (
                        <div className="h-64">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      {disciplineTrend.length === 0 ? (
                        <p className="py-16 text-center text-sm text-zinc-600">Complete daily reviews to build this trend.</p>
                      ) : (
                        <div className="h-64">
                          <ResponsiveContainer width="100%" height="100%">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <Card>
                    ...
                    <CardContent>
                      {skillDistribution.length === 0 ? (
                        <p className="py-16 text-center text-sm text-zinc-600">No skill hours recorded yet.</p>
                      ) : (
                        <>
                          <div className="h-64">
                            <ResponsiveContainer width="100%" height="100%">
                          ...
                          <div className="flex flex-wrap gap-3 mt-4">
                            {skillDistribution.map((item) => (
                              <div key={item.name} className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-zinc-400">{item.name} ({item.value}h)</span>
                              </div>
                            ))}
                          </div>
                        </>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      {failureReasons.length === 0 ? (
                        <p className="py-16 text-center text-sm text-zinc-600">No failures logged yet — keep it up.</p>
                      ) : (
            ...
            <Card>
              ...
              <CardContent>
                {skillProgress.length === 0 ? (
                  <p className="text-sm text-zinc-600 py-4 text-center">No skills configured yet.</p>
                ) : (
                  <div className="space-y-4">
                    {skillProgress.map((skill) => (
                      <div key={skill.name} className="flex items-center gap-4">
                        <div className="w-32 text-sm text-zinc-300 truncate">{skill.name}</div>
                        <div className="flex-1"><Progress value={skill.progress} /></div>
                        <div className="w-12 text-right text-sm text-zinc-500">{skill.progress}%</div>
                        <div className="w-16 text-right text-xs text-zinc-500">{skill.hours}h</div>
                        <div className="w-20 text-right text-xs text-zinc-500">{skill.lastStudied || "-"}</div>
                      </div>
                    ))}
                  </div>
                )}
